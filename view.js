function View(controller) {
	...
	this.createPetHTML = function(pet) {
		...
		var $funInput = $("<input placeholder='Amount (default=10)' type ='text'>");
		var $feedButton = $("<button>Feed!</button>");
		var $foodInput = $("<input placeholder='Amount (default=10)' type ='text'>");
		...
		$playButton.click(function() {
			...
			var amount = 10;
			if ($funInput.val()) {
				amount = parseInt($funInput.val(), 10);
			}
		...
		$feedButton.click(function() {
			var amount = 10; 
			if($foodInput.val()){
				amount = parseInt($foodInput.val(), 10);
			}
	...
	this.createPet = function() {
		...
		controller.beginTimer(name, this.showAllPets.bind(this));
		this.showAllPets();
	}
